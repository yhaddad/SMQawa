...
class bTagCorrector:
    def __init__(self, era:str='2018', wp:str='L', tagger:str='deepJet'):
        ...
        self._tagger = tagger
        self.eff_hist = None
        with gzip.open(f"data/BTV/{era}_UL/eff-btag-{era}.pkl.gz", 'rb') as ifile:
            self.eff_hist = pickle.loads(ifile.read())
        
        b_tag = self.eff_hist[{'tagger':tagger, 'WP': wp, 'passWP': 1  }].values()
        b_all = self.eff_hist[{'tagger':tagger, 'WP': wp, 'passWP': sum}].values()
        ...
        self.eff_statDn = dense_lookup.dense_lookup(dw ,[ax.edges for ax in self.eff_hist.axes[3:]])
        
        self.clib = correctionlib.CorrectionSet.from_file(f"data/BTV/{era}_UL/btagging.json.gz")
    ...
    def btagSF(self, jet, syst="central"):
        ...
        njets = ak.num(jet)
        jet  = ak.flatten(jet)
        sf = self.clib[f"{self.tagger}_comb"].evaluate(
            syst, self._wp, 
