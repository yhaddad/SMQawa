...
class rochester_correction:
    def __init__(self, is_data=False, era:str='2018'):
        ...
        if self._era == '2016APV':
            rochester_data = txt_converters.convert_rochester_file("qawa/data/roccor/RoccoR2016aUL.txt", loaduncs=True)
        elif self._era == '2016':
            rochester_data = txt_converters.convert_rochester_file("qawa/data/roccor/RoccoR2016bUL.txt", loaduncs=True)
        elif self._era == '2017':
            rochester_data = txt_converters.convert_rochester_file("qawa/data/roccor/RoccoR2017UL.txt", loaduncs=True)
        elif self._era == '2018':
            rochester_data = txt_converters.convert_rochester_file("qawa/data/roccor/RoccoR2018UL.txt", loaduncs=True)
        else:
            print('error')
