...
filepath=$2
dataset=$(echo "$filepath" | awk -F'/' '{{print $8}}' | awk -F'_' '{{print $1}}')
runperiod=$(echo "$filepath" | awk -F'/' '{{print $8}}' | awk -F'_' '{{split($2, a, "-"); print substr(a[1], length(a[1]), 1)}}')
python brewer-remote.py --jobNum=$1 --isMC={ismc} --era={era} --infile=$2 --dataset=$dataset --runperiod=$runperiod
...
echo "----- processing the files : "
filepath=$2
dataset=$(echo "$filepath" | awk -F'/' '{{print $8}}')
python brewer-remote.py --jobNum=$1 --isMC={ismc} --era={era} --infile=$2 --dataset=$dataset
...
def main():
    ...
    input_base =f'/eos/user/{user_name[0]}/{user_name}/HZZsample/{options.era}'
