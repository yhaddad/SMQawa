...
class wzinclusive_processor(processor.ProcessorABC):
    ...
    def __init__(self, era: str ='2018', ewk_process_name=None, run_period: str = ''): 
        ...
        self.build_histos = lambda: {
            ...
            'taus_phi': hist.Hist(
                hist.axis.StrCategory([], name="channel"   , growth=True),
                hist.axis.StrCategory([], name="systematic", growth=True), 
                hist.axis.Regular(60, 0, 600, name="taus_phi", label=r"$\phi(\tau)$"),
                hist.storage.Weight()
            ),
            'delta_tau_met_phi': hist.Hist(
                hist.axis.StrCategory([], name="channel"   , growth=True),
                hist.axis.StrCategory([], name="systematic", growth=True), 
                hist.axis.Regular(60, 0, 600, name="delta_tau_met_phi", label=r"$\Delta \phi(\tau, p_{T}^{miss})$"),
                hist.storage.Weight()
            ),
            ...
            'delta_R': hist.Hist(
                hist.axis.StrCategory([], name="channel"   , growth=True),
                hist.axis.StrCategory([], name="systematic", growth=True), 
                hist.axis.Regular(50, 0, 5, name="delta_R", label=r"$\Delta R$"),
                hist.storage.Weight()
            ),
    ...
    def process_shift(self, event, shift_name:str=''):
        ...
        delta_R_lead_tau = ak.fill_none(dilep_p4.delta_r(lead_tau), np.nan)
        delta_R_extra_lep = ak.fill_none(dilep_p4.delta_r(extra_lep), np.nan)
        delta_R = ak.where(ntight_lep==2, dilep_p4.delta_r(lead_tau),dilep_p4.delta_r(lead_tau))
        print("delta_R", delta_R)
        print("delta_R_lead_tau",delta_R_lead_tau)
        print("delta_R_extra_lep",delta_R_extra_lep)
        ...
        #     ak.where(
        #         ngood_jets<2,
        #         ak.fill_none(reco_met_pt > 40, False),
        #         ak.fill_none(reco_met_pt > 40, False)
        #     )
        # )
        ...
        #event['met_pt'  ] = reco_met_pt[ntight_lep==2]
        #print('met_pt = ', event['met_pt'  ])
        #print('before : met_pt = ', reco_met_pt[ntight_lep==2])
        event['met_pt'  ] = reco_met_pt
        #print('after: met_pt = ', event['met_pt'][ntight_lep==2])
        event['m_T'  ] = m_T
        ...
        channels = {
            "inc-SR0": common_sel + [
		    'require-ossf', 'require-2lep', 'dilep_m', 'dilep_dphi_met', 'delta_tau_met_phi', '0njets', '~1nbjets', '1nhtaus', 'met_pt'
        ],
            "inc-SR1": common_sel + [
            'require-ossf', 'require-2lep', 'dilep_m', 'dilep_dphi_met', 'delta_tau_met_phi', '1njets', '~1nbjets', '1nhtaus' ,'met_pt'
        ],
        #     "inc-SR2": common_sel + [
        #     'require-ossf', 'require-2lep', 'dilep_m',
        #     'dilep_dphi_met', 'delta_tau_met_phi', '2njets', '~1nbjets', 
        #     '1nhtaus', 'dilep_dphi_tau', 'met_pt'
        # ],
            "inc-DY": common_sel + [
		    'require-ossf', 'require-2lep', 'dilep_m', 'dilep_dphi_met', 'met_pt','dilep_pt', '1njets' 
        ],
            "inc-DY1": common_sel + [
            'require-ossf', 'require-2lep', 'dilep_m', 'dilep_dphi_met', 'met_pt','dilep_pt', '1njets', '~1nbjets'   
	    ],
            "inc-EM": common_sel + [
        ...
        def _format_variable(variable, cut):
            variable = ak.to_regular(variable, axis=0)
            if cut is None:
                filtered_var = variable
            else:
                filtered_var = variable[cut]
            try:
                vv = ak.fill_none(filtered_var, np.nan)
            except Exception as e:
                print(f"Error filling None values: {e}")
                print("Filtered Variable:", filtered_var)
                return None
            vv_numpy = ak.to_numpy(vv)
            if np.isnan(np.sum(vv_numpy)):
                print(" - vv contains NaN values:", vv_numpy)
        ...
        def _histogram_filler(ch, syst, var, _weight=None):
            ...
            try:
                weight = ak.fill_none(weight, np.nan)
            except Exception as e:
                print(f"Error processing weights for {syst}: {e}")
                weight = np.ones(len(cut))
            
            vv = ak.to_numpy(weight)
            if np.isnan(np.sum(vv)) or np.isinf(np.sum(vv)):
