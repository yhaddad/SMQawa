...
def build_leptons(muons, electrons):
    ...
    loose_electrons = electrons[
        ...
        electrons.mvaIso    # in v9 it was mvaFall17V2Iso_WPL I did not find loose working point in v12
    ]
...
class wzinclusive_processor(processor.ProcessorABC):
    ...
    def __init__(self, era: str ='2022', ewk_process_name=None, run_period: str = ''): 
        ...
        self._leSF = LeptonScaleFactors(era=self._era, isAPV=self._isEE)
        self._jpSF = jetPUScaleFactors(era=self._era, wp=self.jetPU_wp, isAPV=self._isEE)
        self._tauID= tauIDScaleFactors(era=self._era, vsjet_wp=self.tauIDvsjet_wp,vse_wp=self.tauIDvse_wp, vsmu_wp=self.tauIDvsmu_wp, isAPV=self._isEE)
    ...
    def process(self, event: processor.LazyDataFrame):
        ...
        print("---- MET Debug ----")
        print("Raw MET pt:", ak.to_list(raw_met.pt[:5]))
        print("Corrected MET pt:", ak.to_list(met.pt[:5]))
        # print("Number of jets (L123):", ak.num(jets_col_shift_L123, axis=1)[:1])
        # print("Jet pt (first jet, L123):", ak.to_list(jets_col_shift_L123.pt[:1]))
        # print("Jet pt (first jet, L1):", ak.to_list(jets_col_shift_L1.pt[:1]))
        # print("Jet pt diff (L1 - L123):", ak.to_list((jets_col_shift_L1.pt - jets_col_shift_L123.pt)[:1]))
        # print("---------------------")

        # print("Jets before correction (raw):", ak.to_list(event.Jet.pt[:5]))
        # print("Jets after JEC L123:", ak.to_list(jets_shift_L123.pt[:5]))
        # print("Jets after JEC L1:", ak.to_list(jets_shift_L1.pt[:5]))
        # print("Jets after JER:", ak.to_list(jets.pt[:5]))
        ...
        # all_fields = ak.fields(jets)
        # jes_fields = [f for f in all_fields if f.startswith("JES_")]

        # print("Available JES variations on jets:")
        # for field in jes_fields:
        #     print(f"  - {field}")

        print("jet_pt =", jets.pt[:5])
        print("JES_total_up =", jets.JES_Total.up.pt[:5])
        print("JES__total_down =",jets.JES_Total.down.pt[:5])
        print("met_pt = ", met.pt[:5])
        print("MET_total_up =",met.JES_Total.up.pt[:5])
        print("MET_total_down =",met.JES_Total.down.pt[:5])
        # # print("JES_abs_up =", jets.JES_Absolute.up.pt[:5])
        # # print("JES_abs_down =",jets.JES_Absolute.down.pt[:5])
        # # print("JES_bbec1_up =", jets.JES_BBEC1.up.pt[:5])
        # # print("JES_bbec1_down =",jets.JES_BBEC1.down.pt[:5])
        # # print("JES_ec2_down =",jets.JES_EC2.down.pt[:5])
        # # print("JES_hf_up =", jets.JES_HF.up.pt[:5])
        # # print("JES_hf_down =",jets.JES_HF.down.pt[:5])
