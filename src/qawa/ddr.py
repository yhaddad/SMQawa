...
class dataDrivenDYRatio:
	def __init__(self,dilep_pt,met_pt, isDY=False, era:str='2018', ddtype:str='onlySR'):
		...
		if era == '2018' or era=='2017':
			if ddtype == 'MC':
				#print(f"DDMC_Ratio_ptmiss_binned_at_200_{era}_skim20178_DYSR.pkl")
				with open(f"{_data_path}/ddr/DDMC_Ratio_ptmiss_binned_at_200_{era}_skim20178_DYSR.pkl", 'rb') as pkl_file:
					ddmc_ratio = pkl.load(pkl_file)
			else:
				#print(f"{_data_path}/ddr/DDMC_Ratio_ptmiss_binned_at_200_{era}_skim20178_{ddtype}.pkl")
				with open(f"{_data_path}/ddr/DDMC_Ratio_ptmiss_binned_at_200_{era}_skim20178_{ddtype}.pkl", 'rb') as pkl_file:
					ddmc_ratio = pkl.load(pkl_file)
			...
			if ddtype == 'MC':
				...
				with open(f"{_data_path}/ddr/DDMC_Ratio_ptmiss_binned_at_200_2016_skim2016_DYSR.pkl", 'rb') as pkl_file:
					ddmc_ratio = pkl.load(pkl_file)
			else:
				#print(f"{_data_path}/ddr/DDMC_Ratio_ptmiss_binned_at_200_2016_{ddtype}.pkl")
				with open(f"{_data_path}/ddr/DDMC_Ratio_ptmiss_binned_at_200_2016_skim2016_{ddtype}.pkl", 'rb') as pkl_file:
					ddmc_ratio = pkl.load(pkl_file)
