    ...
    with open(options.input, 'r') as stream:
        for sample in stream.read().split('\n'):
            ...
            for idf, rfn in enumerate(input_root_files):
                ...
                elif os.path.exists(jobs_dir + f'/df_{idf}.parquet'):
            ...
            if options.resubmit and len(job_failed)>0: 
                shutil.copyfile('brewer-dump-local.py', jobs_dir+'/brewer-dump-local.py')
                ...
                for jid,infile in resubmit_list.items():
                    ...
                    if options.runlocal:
                        ...
                        local_rerun_lines.append(
                            f"python brewer-dump-local.py --jobNum={jid} --isMC={options.isMC} --era={options.era} --infile={infile_name} --dataset={dataset_name}\n"
                        )
                    else:
                        if options.copyfile:
                            ...
                            infile_name = infile.split('/')[-1]
                            script_command += f"python brewer-dump-local.py --jobNum=$1 --isMC={options.isMC} --era={options.era} --infile={infile_name} --dataset={dataset_name}\n"
                            ...
                            assert options.era != "", f'please specify the era of the dataset you are running. ex: --era=2018'
                            script_command = f"python brewer-dump-local.py --jobNum=$1 --isMC={options.isMC} --era={options.era} --infile=$2\n"
                            script_command += "ls -lthr\n"
                            with open(os.path.join(jobs_dir, f"resub-script-{jid}.sh"), "w") as _stream:
                                script_file_ = resub_script_header.format(
                                    proxy=proxy_copy, 
                ...
                if options.runlocal:
                    ...
                    os.system(f"cp dist/Qawa-{qawa_version}-py2.py3-none-any.whl {jobs_dir}")   
                    if not options.dryrun:
